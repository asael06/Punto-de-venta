...
namespace Punto_de_venta.Ventanas
{
    public partial class Caja : Form
    {
        ...
        public void aceptar()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(txtContrasena.Text) || string.IsNullOrWhiteSpace(txtConcepto.Text)) this.Close();
                else
                {
                    int[] ids = BDIngresos.ArrIngresosId().ToArray();
                    Ingresos ing = new Ingresos(regresarId(ids, BDIngresos.idMax()), txtConcepto.Text, -Convert.ToDouble(txtContrasena.Text), DateTime.Now.Date.ToString("yyyy-MM-dd"), BDLogeado.retornaLogeado());
        ...
        private void txtContrasena_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            validation.DesactivarTeclado(txtContrasena, 10, e);
        }
    }
}